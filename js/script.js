...
function atualizarCard() {
  document.querySelector(".itens-carrinhos-compra span").innerHTML =
    cart.length;
  ...
  for (let i in cart) {
    ...
    let ItemDoCart = document
      .querySelector(".modelo-produto .container-cards-produtos")
      .cloneNode(true);

    ItemDoCart.querySelector("img").src = produtosAdicionado.img;
    ItemDoCart.querySelector(".card-produto-nome").innerHTML =
      produtosAdicionado.name;
    ItemDoCart.querySelector(".card-item-quantidade").innerHTML = cart[i].qt;
    ItemDoCart.querySelector(".quantidade-menos").addEventListener(
      "click",
      () => {
        if (cart[i].qt > 1) {
          cart[i].qt--;
    ...
    ItemDoCart.querySelector(".quantidade-mais").addEventListener(
      "click",
      () => {
        cart[i].qt++;
        atualizarCard();
